...
const WeeklyChart = ({ entries }) => {
    ...
    const dailyTotals = streamTally.map(s => s.clientWork + s.practiceDevelopment + s.businessDevelopment);
    const maxTotal = Math.max(...dailyTotals, 1);

    return (
        <Box sx={{ height: '220px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 1.5, px: 2, pt: 2 }}>
            {streamTally.map((streams, index) => (
                <Stack key={index} spacing={1} alignItems="center" sx={{ flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                    <Tooltip 
                        title={
                            <Box sx={{ p: 1 }}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 900 }}>{days[index]}</Typography>
                                <Typography variant="body2" sx={{ color: STREAM_COLORS.clientWork }}>CW: {streams.clientWork} words</Typography>
                                <Typography variant="body2" sx={{ color: STREAM_COLORS.practiceDevelopment }}>PD: {streams.practiceDevelopment} words</Typography>
                                <Typography variant="body2" sx={{ color: STREAM_COLORS.businessDevelopment }}>BD: {streams.businessDevelopment} words</Typography>
                            </Box>
                        } 
                        arrow 
                        placement="top"
                    >
                        <Box sx={{ 
                            width: '100%', 
                            maxWidth: '45px', 
                            height: dailyTotals[index] > 0 ? `${(dailyTotals[index] / maxTotal) * 100}%` : '6px',
                            display: 'flex',
