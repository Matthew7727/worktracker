...
describe('EntryCard Component', () => {
  ...
  it('renders rendered markdown in preview mode by default for saved entries', () => {
    render(<EntryCard entry={mockEntry} {...mockHandlers} />, {
      wrapper: TestWrapper,
    })
    ...
    expect(
      screen.getByText(/testing/i, { selector: 'strong' })
    ).toBeInTheDocument()
  ...
  it('switches to edit mode when the Edit button is clicked', async () => {
    ...
    expect(
      screen.getByPlaceholderText('Describe what you accomplished...')
    ).toBeInTheDocument()
    expect(
      screen.getByDisplayValue('I worked on **testing** today.')
    ).toBeInTheDocument()
  })
  ...
  it('shows markdown toolbar when in edit mode', () => {
    ...
    render(<EntryCard entry={newEntry} {...mockHandlers} />, {
      wrapper: TestWrapper,
    })
  ...
  it('triggers onUpdateContent when text is typed in edit mode', () => {
    ...
    expect(mockHandlers.onUpdateContent).toHaveBeenCalledWith(
      'test-id',
      'New content'
    )
  })
