...
export const injectMarkdown = (text, start, end, type) => {
  ...
  switch (type) {
    ...
    case 'heading': {
      ...
      newCursor =
        start + hPrefix.length + (selectedText ? selectedText.length : 7)
      break
    }
    case 'list': {
      ...
      newCursor =
        start + lPrefix.length + (selectedText ? selectedText.length : 9)
      break
    }
    case 'blockquote': {
      ...
      newCursor =
        start + qPrefix.length + (selectedText ? selectedText.length : 5)
      break
    }
    case 'code': {
      if (selectedText.includes('\n') || !selectedText) {
        ...
        newCursor =
          start + cPrefix.length + (selectedText ? selectedText.length : 10)
      } else {
        // Inline code
