...
describe('GlobalSearch Component', () => {
  ...
  it('opens search dialog when clicking the search icon', async () => {
    render(
      <GlobalSearch rootDir={rootDir} onResultClick={mockOnResultClick} />,
      { wrapper: TestWrapper }
    )
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByPlaceholderText(/type keywords to find logs/i)
      ).toBeInTheDocument()
    })
  })
  ...
  it('calls onResultClick when a result is selected', async () => {
    window.electronAPI.searchEntries.mockResolvedValue({
      success: true,
      results: [
        { date: '2024-05-20', snippet: 'Selected', fileName: '2024-05-20.md' },
      ],
    })
